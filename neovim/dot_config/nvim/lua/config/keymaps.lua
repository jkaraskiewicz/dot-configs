...
map('n', '<leader>ra', function() vim.lsp.buf.code_action() end, { desc = 'Code actions' })
map('n', '<leader>re', function() require('mini.extra').pickers.diagnostic({ scope = 'current' }) end,
  { desc = 'LSP errors in buffer' })
map('n', '<leader>rq', function() vim.diagnostic.setqflist({ open = true }) end, { desc = 'LSP quickfix' })
